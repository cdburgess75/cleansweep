...
<#
.SYNOPSIS
    Dave's CleanSweep v0.51 — Enterprise PUP, Adware & Malware IOC Remediation Tool
...
if ($Script:filenameIOCList.Count -gt 0) {
    try {
        $dynPattern = ($Script:filenameIOCList | ForEach-Object { [regex]::Escape($_) }) -join '|'
        ...
        Log-Info "Filename IOCs loaded: $($Script:filenameIOCList.Count) entries (source: $fileSource) — regex compiled"
    } catch {
